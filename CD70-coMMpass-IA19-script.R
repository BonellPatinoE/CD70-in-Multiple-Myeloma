...
temp_bar <- data.frame(y = as.numeric(temp[temp$GENE %in% "CD70", 1:203]), 
                       x = x,                
                       stringsAsFactors = F)
temp_bar
...
# PLOT variance of CD70 expression across every sample
temp_bar%>%ggplot(aes(x = x, y = y)) + 
    ...
    ggtitle(label = "CD70", 
            subtitle = paste0("log2FC = ",
                              round(res$log2FoldChange[res$GENE == "CD70"], 3),
                              "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
...
temp%>%filter(GENE == "CD70")
names(temp)
temp[temp$GENE == "CD70", 1:162]
x<-str_extract(names(temp), "_[0-9]_")[1:162] 
x<-str_extract(x, "[0-9]")[1:162]
x

temp_bar <- data.frame(y = as.numeric(temp[temp$GENE %in% "CD70", 1:162]), 
                       x = x,                
                       stringsAsFactors = F)
temp_bar
...
temp_bar1%>%ggplot(aes(x = x, y = y)) + 
  ...
  ggtitle(label = "CD70", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE %in% "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE %in% "CD70"], 3))) +
  theme_minimal()
...
plotMA(resLFC1, ylim=c(-2,2), main = " MA plot ISS 2 vs 1 LFC shrink")
  ...
  temp_flip<-temp%>%filter(GENE == "CD70")
  head(temp_flip)
...
temp_bar2%>%ggplot(aes( x = NDS2, y = temp_flip))+ 
  ...
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on t(4:14) [NDS2] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
...
class(res)
res$log2FoldChange
res%>%filter(GENE == "CD70")
res$log2FoldChange[res$GENE == "CD70"]
res$pvalue[res$GENE == "CD70"]
round(res$log2FoldChange[res$GENE == "CD70"])
res$log2FoldChange[res$GENE == "CD70"]
...
temp_bar2%>%ggplot(aes( x = CCND1, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(11:14) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on t(11:14) [CCND1] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"],3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"],3))) +
  ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
...
temp_bar2%>%ggplot(aes( x = CCND3, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(6:14) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on t(6:14) [CCND3] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
...
temp_bar2%>%ggplot(aes( x = MYC, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "MYC mutation",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on MYC mutation", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = MAF, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(14;16) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on t(14;16) [c-MAF] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = MAFB, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(14;20) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on t(14;20) [MAFB] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = del17p13, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "del17p13 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on del17p13 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = TP53, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "TP53 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on TP53 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = Gain1q21, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "Gain1q21 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on Gain1q21 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
...
temp_bar2%>%ggplot(aes( x = del1p22, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "del1p22 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on del1p22 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
...
temp_bar2%>%ggplot(aes( x = MYC, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "MYC Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CD70 expression based on MYC status (SeqFISH)", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CD70"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CD70"], 3))) +
  ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
